...
import { Divider, InputNumber, Button, message } from "antd";
...
const NewPosition = ({ user, poolV2, dispatch }) => {
    ...
    const handleToken1Change = (value) => {
        ...
        const { sqrtPriceX96, liquidity, tick, token1: { decimal } } = poolV2.curPair
        const sqrtPrice1 = getSqrtRatioAtTick(tickLower)
        const sqrtPrice2 = getSqrtRatioAtTick(tickUpper)
        const liquidityAmount = getMaxLiquidityForAmounts(BigInt(sqrtPriceX96), sqrtPrice1, sqrtPrice2, BigInt(value * 10 ** decimal), BigInt(18446744073709551616))
        ...
        setToken2(formatSat(swapRes.amount1, decimal))
    }
    const handleToken2Change = (value) => {
        if (!(poolV2.curPair && poolV2.curPair.tickSpacing)) return;
        const { sqrtPriceX96, liquidity, tick, token2: { decimal } } = poolV2.curPair
        // setToken2(value)
        const sqrtPrice1 = getSqrtRatioAtTick(tickLower)
        const sqrtPrice2 = getSqrtRatioAtTick(tickUpper)
        const liquidityAmount = getMaxLiquidityForAmounts(BigInt(sqrtPriceX96), sqrtPrice1, sqrtPrice2, BigInt(18446744073709551616), BigInt(value * 10 ** decimal),)
        const swapRes = mint(Number(tick), BigInt(sqrtPriceX96), BigInt(liquidity), tickLower, tickUpper, liquidityAmount);
        setLiquidityAmount(liquidityAmount)
        setToken1(formatSat(swapRes.amount0, decimal))
        setToken2(formatSat(swapRes.amount1, decimal))
    ...
    return <PageContainer>
        <div className="newPositionPage">
            ...
            <div className="FormItemTitle">Amount</div>

            <div className="inputWrap">
                <div className="label">
                    token1
                </div>
                <InputNumber onBlur={(e) => { handleToken1Change(e.target.value) }} value={token1} bordered={false} controls={false} ></InputNumber>
            </div>
            <div className="inputWrap">
                <div className="label">
                    token2
                </div>
                <InputNumber value={token2} bordered={false} controls={false} onChange={handleToken2Change}></InputNumber>
            </div>
            <Button onClick={handleCreatePosition}>Add Liq</Button>
